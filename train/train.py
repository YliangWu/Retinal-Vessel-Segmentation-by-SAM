...
def get_args_parser():
    ...
    parser.add_argument("--output", type=str, default="./output/" + run_id,
    ...
    print("restore_model",parser.parse_args().restore_model)
...
def main(net, train_datasets, valid_datasets, args):
    ...
    if not args.eval:
        ...
        optimizer = optim.Adam(net_without_ddp.parameters(), lr=args.learning_rate, betas=(0.9, 0.999), eps=1e-08,
                               weight_decay=0)
        # lr_drop_epoch =2
        # lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, lr_drop_epoch)
        ...
        """
        lr_scheduler = CosineAnnealingWarmRestarts(optimizer=optimizer, T_0=5, T_mult=2, eta_min=1e-5, verbose=True)
        lr_scheduler.last_epoch = args.start_epoch
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num):
        ...
        for data in metric_logger.log_every(train_dataloaders, 10):
            ...
            loss_mask, loss_dice = loss_masks(masks_hq, labels , len(masks_hq)) #masks_hq: batch_size,1,256,256   labels:batch_size,1,1024,1024
            loss = loss_mask + loss_dice
            print('Time: {},epoch:{}-{}: loss:{}'.format(datetime.now().strftime("%Y%m%d-%H%M"), epoch + 1, step + 1,
                                                         loss.item()))
            step += 1
        ...
        losses.append(loss_per_batch / num)
        print("Finished epoch:      ", epoch)
        metric_logger.synchronize_between_processes()
        print("train_Averaged stats:", metric_logger)
        train_stats = {k: meter.global_avg for k, meter in metric_logger.meters.items() if meter.count > 0}

        lr_scheduler.step()
        # 测试
        test_stats = evaluate(args, net, sam, valid_dataloaders, visualize=False)
        train_stats.update(test_stats)

        net.train()
        #save了mask_decoder的backbone
    ...
    # 训练完所有轮次,eval一下
    test_stats = evaluate(args, net, sam, valid_dataloaders, visualize=True)
    train_stats.update(test_stats)

    # merge sam and hq_decoder
    if misc.is_main_process():
        #再存一下mask_decoder
        model_name = "/mask_decoder_epoch_" + str(epoch + 1) + ".pth"
        print('come here save at', args.output + model_name)
        misc.save_on_master(net.state_dict(), args.output + model_name)
        #这里直接用sam是不是也一样
        sam_ckpt = torch.load(args.checkpoint)  #读取sam_backbond的文件
        hq_decoder = torch.load(args.output + model_name) #读取刚刚输出的maskdecoder文件
        for key in hq_decoder.keys():
            sam_key = 'mask_decoder.' + key
            if sam_key not in sam_ckpt.keys():
                sam_ckpt[sam_key] = hq_decoder[key]
                """经过 if 的筛选，最终以最小占用内存方式存储了hq模型"""
        model_name = "/sam_hq_epoch_" + str(epoch+1) + ".pth"
    ...
    plt.tight_layout()
    plt.savefig(args.output + "/imfo.png")
    #plot_log(args.output)
...
if __name__ == "__main__":
    ...
    dataset_train = {"name": "CHASEDB1",
                     "im_dir": "./CHASEDB1/train/img",
                     "gt_dir": "./CHASEDB1/train/valid_mask",
                     "im_ext": ".jpg",
                     "gt_ext": ".jpg"}

    dataset_eval = {
        "name": "CHASEDB1",
        "im_dir": "./CHASEDB1/test/img",
        "gt_dir": "./CHASEDB1/test/valid_mask",
        "im_ext": ".jpg",
        "gt_ext": ".jpg"
    }

    train_datasets = [dataset_train]
    valid_datasets = [dataset_eval]
