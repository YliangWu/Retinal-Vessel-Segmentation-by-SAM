...
def get_args_parser():
    ...
    parser.add_argument("--restore_model", type=str, default="./pretrained_checkpoint/mask_decoder.pth",
...
def predict(args,valid_datasets,visualize):
    ...
    return masks,gt##返回1024*1024的mask图
...
if __name__ == "__main__":
    ...
    dataset_eval = {
        ...
        "gt_dir": "./CHASEDB1/test/valid_mask", #文件路径
        "im_ext": ".jpg",   #文件类型
        "gt_ext": ".jpg"    #groundtruth文件类型
    }
    valid_datasets = [dataset_eval]
    args = get_args_parser()
    """输出：列表 masks(True-False),gt(0-255),尺寸1024*1024 一一顺序一一对应 """
    with torch.no_grad():
        masks,ground_truth=predict(args,valid_datasets, visualize=True) ##visualize=True可以输出预测结果图到文件夹中
    ...
    for i  in range(len(masks)):
        ...
        print(mask)
        plt.figure()
        show_mask(mask,plt.gca(), random_color=False) #输出
        plt.show()
        plt.figure()
        show_mask(gt/255,plt.gca(),random_color=False)
        plt.show()
