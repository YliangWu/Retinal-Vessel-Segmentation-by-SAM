...
def create_dataloaders(name_im_gt_list, my_transforms=[], batch_size=1, training=False):
    ...
    if training:
        ...
        num_samples = len(gos_dataset)
        print("size of the dataset={}".format(num_samples))
        #sampler = DistributedSampler(gos_dataset)
        sampler = RandomSampler(gos_dataset,num_samples=num_samples,replacement=True) #有放回采样
...
def dataloaders2(name_im_gt_list, my_transforms=[], batch_size=1, training=False):
    gos_dataloaders = []
    gos_datasets = []
